...
const pickNodeBySourceUrlOrCheerioImg = ({
  ...
  const imageNode = mediaItemNodes.find(mediaItemNode => {
    ...
    return (// either find our node by the source url
      ...
      (_ref = mediaItemNode.sourceUrl || mediaItemNode.mediaItemUrl) === null || _ref === void 0 ? void 0 : _ref.replace(`-scaled`, ``)) || !!cheerioImg && getCheerioImgRelayId(cheerioImg) === mediaItemNode.id
    );
  });
  return imageNode;
};
    ...
    const htmlMatchesWithImageResizes = await Promise.all(imgTagMatches.map(async ({
      ...
      if (gatsbyTransformerSharpSupportsThisFileType) {
        try {
          ...
          if (pluginOptions !== null && pluginOptions !== void 0 && (_pluginOptions$html5 = pluginOptions.html) !== null && _pluginOptions$html5 !== void 0 && _pluginOptions$html5.generateWebpImages) {
            const webpResult = await (0, _gatsbyPluginSharp.fluid)({
              file: fileNode,
              args: {
                maxWidth,
                quality,
